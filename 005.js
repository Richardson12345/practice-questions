...
function simpleCardsFusion (str) {
  ...
  const fusionCards = [
    ...
    for(j = 0; j < fusionCards.length; j++){
      newObj.push({})
    }
    for(z = 0; z < fusionCards.length; z ++){
      newObj[z].card1 = fusionCards[z].card1 + '+' + fusionCards[z].card2;
      newObj[z].card2 = fusionCards[z].card2 + '+' + fusionCards[z].card1;
      newObj[z].name = fusionCards[z].name;
      }
      
      
      
  for(z = 0; z < newObj.length; z ++){
      if(newObj[z].card1 === str || newObj[z].card2 === str){
          return newObj[z].name;
          }
        }
